...
/* ─────────────────────────────────────────
   ...
   Actions are defined in plutocrat-engine-actions.js.
   This stub ensures PG exists before screens reference it.
─────────────────────────────────────────── */
window.PG={closeModal:closeModal};

/* ─────────────────────────────────────────
   EXPOSE INTERNALS FOR OTHER ENGINE FILES
   Screens and actions are in separate files but run inside the
   same IIFE scope because they are all injected here via
   plutocrat-engine-screens.js and plutocrat-engine-actions.js.

   IMPORTANT: The three engine files share one IIFE.
   This file opens it. The other two files continue inside it.
   plutocrat-engine-actions.js closes the IIFE.

   All internal functions are therefore in the same closure.
─────────────────────────────────────────── */
...
/* ─────────────────────────────────────────
   EXPOSE CORE INTERNALS TO OTHER ENGINE FILES
   All three engine files share the same IIFE closure.
   These are the functions screens and actions will call.
─────────────────────────────────────────── */
/* These are accessed by engine-screens and engine-actions
   because all three files are concatenated inside one IIFE.
   See IMPORTANT note above. */

/* Export init trigger for the actions file */
window._pgInit=init;
